...
export class PersonaService {
   ...
   */
  async generatePersonaAndGoals(userId: string): Promise<{ persona: string; goals: string[] }> {
    // 1. 프로필, 관심사, 라이프스타일, 최근 일기/피드백 등 데이터 수집
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      include: { interests: true },
    });
    const lifestyle = await this.prisma.lifestyleAnswer.findMany({ where: { userId } });
    const diaries = await this.prisma.journal.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, take: 5 });
    // 2. 프롬프트 구성
    ...
    const persona = /---페르소나---([\s\S]*?)---/g.exec(text)?.[1]?.trim() || '';
    const goals = /---목표---([\s\S]*)/g.exec(text)?.[1]?.split(/\n|,|-/).map(s => s.trim()).filter(Boolean) || [];
    return { persona, goals };
  }

  private buildPersonaPrompt(user: any, lifestyle: any[], diaries: any[]): string {
