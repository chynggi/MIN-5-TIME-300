...
export class AdviceController {
  ...
  async latest(@Req() req: any) {
    ...
    this.rate.setCache(cacheKey, latest ?? { advice: '작게 시작해도 좋아요—오늘은 5분만 쉬어가요.', risk_flag: 'none', tags: ['루틴'] });
    return latest ?? { advice: '작게 시작해도 좋아요—오늘은 5분만 쉬어가요.', risk_flag: 'none', tags: ['루틴'] };
  }
  ...
  async generate(@Req() req: any, @Query('force') force?: string) {
    ...
    if (!this.rate.isAllowed(`advice_gen:${userId}`, 3)) {
      return { ok: false, message: '요청이 너무 잦아요. 잠시 후 다시 시도해 주세요.' };
    }
  ...
  @Post('feedback')
  async feedback(@Req() req: any, @Body() body: { adviceId: string; helpful: boolean }) {
    const userId = req.user.userId;
    if (!body?.adviceId || typeof body.helpful !== 'boolean') {
      return { ok: false, message: 'adviceId와 helpful(boolean)가 필요합니다.' };
    }
    const res = await this.service.feedback(userId, body.adviceId, body.helpful);
    return { ok: true, ...res };
  }
